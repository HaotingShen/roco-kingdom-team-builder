...
export function buildShareUrl(team: TeamOut, lang: "en" | "zh", includeUsername?: string, note?: string): string {
  const base = import.meta.env.VITE_ASSET_BASE_URL || window.location.origin;
  return `${base}/${lang}/import?t=${encodeSharePayload(team, includeUsername, note)}`;
}
