...
def collect(db) -> dict:
    ...
    # ── 0. Scope ─────────────────────────────────────────────────────────────
    # Exclude featured (admin-curated) teams and teams whose monster composition
    # matches a featured team (to remove "快速组队" copies saved by users).
    # All valid saved teams have exactly 6 monsters (enforced by frontend before save).

    # Step 1: get featured team monster compositions (sorted monster_id combos)
    featured_combos_q = db.execute(text("""
        SELECT STRING_AGG(monster_id::text, ',' ORDER BY monster_id) AS combo
        FROM user_monsters
        WHERE team_id IN (SELECT id FROM teams WHERE is_featured = true)
        GROUP BY team_id
    """)).fetchall()
    ...
    featured_copies_filtered = len(user_teams_q) - len(user_team_ids)
    ...
    data["scope"] = {
        ...
        "featured_copies_filtered": featured_copies_filtered,
        "total_users": db.query(func.count(User.id)).scalar(),
        "total_users_with_teams": db.query(func.count(func.distinct(Team.owner_id)))
            .filter(Team.id.in_(user_team_ids)).scalar() if user_team_ids else 0,
    ...
    if not user_team_ids:
        # No qualifying teams — fill empty placeholders and return early
        for key in [
            "monster_usage", "species_usage", "pairs", "trios", "magic_items",
            "moves", "personalities", "per_monster_personality", "legacy_types",
            "per_monster_legacy", "per_monster_moves", "type_coverage",
            "talent_freq", "talent_spread", "talent_avg", "teams_per_user",
            "meta_teams_loose", "meta_teams_exact", "tier_dist", "lang_pref",
        ]:
            data[key] = []
        data["talent_freq"] = None
        data["talent_avg"] = None
        data["_total_user_teams"] = 0
    ...
    # All display names use Chinese localized name (COALESCE falls back to English).
    monster_usage = db.execute(text("""
        SELECT
            m.id,
            COALESCE(m.localized->'zh'->>'name', m.name) AS name,
            m.form,
            COALESCE(ms.localized->'zh'->>'name', ms.name) AS species_name,
            COALESCE(t1.localized->'zh'->>'name', t1.name) AS main_type,
            COALESCE(t2.localized->'zh'->>'name', t2.name) AS sub_type,
        ...
        GROUP BY m.id, m.name, m.localized, m.form, ms.id, ms.name, ms.localized,
                 t1.id, t1.name, t1.localized, t2.id, t2.name, t2.localized
    ...
    species_usage = db.execute(text("""
        ...
        JOIN monster_species ms ON m.species_id = ms.id
        WHERE um.team_id = ANY(:ids)
        GROUP BY ms.id, ms.name, ms.localized
        ORDER BY usage_count DESC
    """), {"ids": user_team_ids}).fetchall()
    ...
    # ── 4. Monster trios ──────────────────────────────────────────────────────
    # Canonicalize trio order by monster ID: min_id → mid_id → max_id.
    trios = db.execute(text("""
        SELECT mon_a, mon_b, mon_c, trio_count
        FROM (
            SELECT
                CASE WHEN m1.id = LEAST(m1.id,m2.id,m3.id)    THEN COALESCE(m1.localized->'zh'->>'name', m1.name)
                     WHEN m2.id = LEAST(m1.id,m2.id,m3.id)    THEN COALESCE(m2.localized->'zh'->>'name', m2.name)
                     ELSE COALESCE(m3.localized->'zh'->>'name', m3.name) END AS mon_a,
                CASE WHEN m1.id = GREATEST(m1.id,m2.id,m3.id) THEN COALESCE(m1.localized->'zh'->>'name', m1.name)
                     WHEN m2.id = GREATEST(m1.id,m2.id,m3.id) THEN COALESCE(m2.localized->'zh'->>'name', m2.name)
                     ELSE COALESCE(m3.localized->'zh'->>'name', m3.name) END AS mon_c,
                CASE WHEN m1.id NOT IN (LEAST(m1.id,m2.id,m3.id), GREATEST(m1.id,m2.id,m3.id))
                         THEN COALESCE(m1.localized->'zh'->>'name', m1.name)
                     WHEN m2.id NOT IN (LEAST(m1.id,m2.id,m3.id), GREATEST(m1.id,m2.id,m3.id))
                         THEN COALESCE(m2.localized->'zh'->>'name', m2.name)
                     ELSE COALESCE(m3.localized->'zh'->>'name', m3.name) END AS mon_b,
                COUNT(*) AS trio_count
            FROM user_monsters um1
            ...
            WHERE um1.team_id = ANY(:ids)
            GROUP BY LEAST(m1.id,m2.id,m3.id), GREATEST(m1.id,m2.id,m3.id),
                     m1.id, m1.name, m1.localized,
                     m2.id, m2.name, m2.localized,
                     m3.id, m3.name, m3.localized
        ) ranked
    ...
    magic_items = db.execute(text("""
        SELECT
            COALESCE(mi.localized->'zh'->>'name', mi.name) AS name,
            COUNT(t.id) AS usage_count,
            COUNT(t.id) * 100.0 / :total AS pct
        FROM teams t
        JOIN magic_items mi ON t.magic_item_id = mi.id
        WHERE t.id = ANY(:ids)
        GROUP BY mi.id, mi.name, mi.localized
        ORDER BY usage_count DESC
    ...
    moves_pop = db.execute(text("""
        SELECT
            COALESCE(mv.localized->'zh'->>'name', mv.name) AS name,
            COALESCE(mt.localized->'zh'->>'name', mt.name) AS move_type,
            mv.move_category,
            mv.energy_cost,
            mv.power,
            COUNT(*) AS slot_usage,
        ...
        WHERE um.team_id = ANY(:ids)
        GROUP BY mv.id, mv.name, mv.localized, mt.id, mt.name, mt.localized,
                 mv.move_category, mv.energy_cost, mv.power
    ...
    personalities = db.execute(text("""
        SELECT
            COALESCE(p.localized->'zh'->>'name', p.name) AS name,
            COUNT(um.id) AS usage_count
        FROM user_monsters um
        JOIN personalities p ON um.personality_id = p.id
        WHERE um.team_id = ANY(:ids)
        GROUP BY p.id, p.name, p.localized
        ORDER BY usage_count DESC
    """), {"ids": user_team_ids}).fetchall()
    data["personalities"] = personalities

    # ── 8. Per-monster personality breakdown (top 20 monsters only) ───────────
    # Which personality does each popular monster most often use?
    # Partition/group by monster ID (stable) and display Chinese names.
    per_monster_personality = db.execute(text("""
        SELECT monster_name, personality, cnt, rnk FROM (
            SELECT
                COALESCE(m.localized->'zh'->>'name', m.name) AS monster_name,
                COALESCE(p.localized->'zh'->>'name', p.name) AS personality,
                COUNT(*) AS cnt,
                SUM(COUNT(*)) OVER (PARTITION BY m.id) AS monster_total,
                RANK() OVER (PARTITION BY m.id ORDER BY COUNT(*) DESC) AS rnk
            FROM user_monsters um
            ...
            GROUP BY m.id, m.name, m.localized, p.id, p.name, p.localized
        ) sub
        WHERE rnk <= 3
    ...
    legacy_types = db.execute(text("""
        SELECT
            COALESCE(t.localized->'zh'->>'name', t.name) AS legacy_type,
            COUNT(um.id) AS usage_count
        FROM user_monsters um
        JOIN types t ON um.legacy_type_id = t.id
        WHERE um.team_id = ANY(:ids)
        GROUP BY t.id, t.name, t.localized
        ORDER BY usage_count DESC
    """), {"ids": user_team_ids}).fetchall()
    ...
    per_monster_legacy = db.execute(text("""
        SELECT monster_name, legacy_type, cnt, rnk FROM (
            ...
            WHERE um.team_id = ANY(:ids)
              AND m.id IN (
                SELECT m2.id FROM user_monsters um2
                JOIN monsters m2 ON um2.monster_id = m2.id
                WHERE um2.team_id = ANY(:ids)
                GROUP BY m2.id ORDER BY COUNT(*) DESC LIMIT 20
              )
            GROUP BY m.id, m.name, m.localized, t.id, t.name, t.localized
        ) sub
        WHERE rnk <= 3
        ORDER BY monster_total DESC, monster_name, rnk
    """), {"ids": user_team_ids}).fetchall()
    ...
    per_monster_moves = db.execute(text("""
        SELECT monster_name, move_name, move_type, move_category, cnt, rnk FROM (
            SELECT
                COALESCE(m.localized->'zh'->>'name', m.name) AS monster_name,
                COALESCE(mv.localized->'zh'->>'name', mv.name) AS move_name,
                COALESCE(mt.localized->'zh'->>'name', mt.name) AS move_type,
                mv.move_category,
                COUNT(*) AS cnt,
                SUM(COUNT(*)) OVER (PARTITION BY m.id) AS monster_total,
            ...
            GROUP BY m.id, m.name, m.localized,
                     mv.id, mv.name, mv.localized, mt.id, mt.name, mt.localized,
                     mv.move_category
        ) sub
        WHERE rnk <= 5
        ORDER BY monster_total DESC, monster_name, rnk
    ...
    type_coverage2 = db.execute(text("""
        ...
        FROM (
            ...
            FROM (
                SELECT DISTINCT um.team_id,
                    COALESCE(t.localized->'zh'->>'name', t.name) AS type_name
                FROM user_monsters um
                JOIN monsters m ON um.monster_id = m.id
    ...
    # Canonicalize by sorting on monster ID (stable integer) for STRING_AGG order.
    meta_teams_loose = db.execute(text("""
        SELECT monster_combo, COUNT(*) AS team_count
        FROM (
            SELECT team_id,
                STRING_AGG(
                    COALESCE(m.localized->'zh'->>'name', m.name),
                    ',' ORDER BY m.id
                ) AS monster_combo
            FROM user_monsters um
            JOIN monsters m ON um.monster_id = m.id
    ...
    # Slots are canonicalized by sorting on monster ID so order doesn't matter.
    meta_teams_exact = db.execute(text("""
        SELECT full_combo, magic_item, COUNT(*) AS team_count
        FROM (
            SELECT
                t.id AS team_id,
                COALESCE(mi.localized->'zh'->>'name', mi.name) AS magic_item,
                STRING_AGG(
                    COALESCE(m.localized->'zh'->>'name', m.name) || '|' ||
                    COALESCE(p.localized->'zh'->>'name', p.name) || '|' ||
                    COALESCE(lt.localized->'zh'->>'name', lt.name) || '|' ||
                    COALESCE(mv1.localized->'zh'->>'name', mv1.name, '') || '|' ||
                    COALESCE(mv2.localized->'zh'->>'name', mv2.name, '') || '|' ||
                    COALESCE(mv3.localized->'zh'->>'name', mv3.name, '') || '|' ||
                    COALESCE(mv4.localized->'zh'->>'name', mv4.name, ''),
                    ';;'
                    ORDER BY m.id, p.id
                ) AS full_combo
            FROM teams t
            ...
            WHERE t.id = ANY(:ids)
            GROUP BY t.id, mi.id, mi.name, mi.localized
        ) combos
...
def render(data: dict) -> str:
    ...
    p(f"| Teams filtered (copies of featured compositions) | {s['featured_copies_filtered']} |")
    ...
    p(f"| Users who have saved ≥1 qualifying team | {s['total_users_with_teams']} |")
    blank()
    ...
    blank()
    p("| Rank | Magic Item | # Teams | % of all teams |")
    p("| --- | --- | --- | --- |")
    for i, r in enumerate(data["magic_items"], 1):
        p(f"| {i} | {r.name} | {r.usage_count} | {float(r.pct):.1f}% |")
    blank()
    ...
    h2("11a. Language preference (registered users only)")
    p("| Language | # Users |")
    p("| --- | --- |")
    ...
    if ml:
        p("| Rank | Monsters | # Teams |  % of all teams |")
        p("| --- | --- | --- | --- |")
...
if __name__ == "__main__":
    ...
    print(f"  Featured copies filtered: {data['scope']['featured_copies_filtered']}")
